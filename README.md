...
The server runs as one long-lived Bun process. Agents connect to `/mcp` with bearer tokens. Each token maps to one agent identity and workspace, and the browser dashboard manages those access keys.

## Features

- **Streamable HTTP MCP** with stateful sessions at `/mcp`.
- **Dashboard UI** for access keys, online agents, recent tasks, active locks, pinned notes, and workspaces.
...
Use the admin token from `AGENT_MAILBOX_ADMIN_TOKEN` to sign in, create access keys, then give each agent its own bearer token.

## Scripts

| Script | Description |
| --- | --- |
| `bun run http` | Start the long-running Streamable HTTP MCP server and dashboard. |
| `bun run build` | Build the React dashboard into `public/`. |
| `bun run dev` | Start the Vite dashboard dev server with `/api` proxied to `127.0.0.1:8137`. |
...
The dashboard is a Vite React app in `ui/`. It uses Tailwind CSS and shadcn/ui-style copied components configured by `components.json`.

Important paths:

- `ui/src/components/ui`: local shadcn primitives.
- `ui/src/components`: dashboard sections built from those primitives.
- `ui/src/lib/utils.ts`: shared `cn()` helper used by shadcn components.
- `public/`: production assets generated by `bun run build` and served by `bun run http`.

## HTTP Configuration

| Variable | Required | Description |
| --- | --- | --- |
| `AGENT_MAILBOX_ADMIN_TOKEN` | Yes | Dashboard and admin API bearer token. |
...
The dashboard is the normal way to create keys. For deployments that need initial keys without using the UI first, set `AGENT_MAILBOX_HTTP_TOKENS`:
...
Use Streamable HTTP support in your MCP client and pass a bearer token created in the dashboard.
...
- `GET /api/dashboard`: dashboard summary, keys, agents, tasks, locks, notes, and workspaces.
